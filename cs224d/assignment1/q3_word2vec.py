...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    gradPred = dscores.dot(outputVectors)     # 1xV x VxD = 1xD
    grad = dscores.reshape(V, 1).dot(predicted.reshape(D, 1).T)   # Vx1 x 1xD = VxD
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    w_r_out = sigmoid(outputVectors[target].dot(predicted))  # 1xD x Dx1 = 1x1
    w_r_k = sigmoid(- sampleVectors.dot(predicted))          # KxD x Dx1 = Kx1
